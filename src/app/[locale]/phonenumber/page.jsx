...
export default function PhoneNumberPage() {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-obsidian px-4 py-10">
      <div className="w-full max-w-md animate-rise rounded-3xl border border-[var(--line)] bg-white/[0.03] p-8">
        {step === 'phone' ? (
          <>
            ...
            <form className="mt-8 space-y-5" onSubmit={handleSubmitPhone}>
              ...
              <button
                type="submit"
                disabled={loading}
                className="group relative w-full overflow-hidden rounded-2xl bg-gradient-passion px-5 py-3 text-white shadow-[0_10px_34px_-10px_rgba(246,65,53,0.5)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_50px_-12px_rgba(235,6,3,0.45)] disabled:pointer-events-none disabled:opacity-60"
              >
          ...
          <>
            ...
            <form className="mt-8 space-y-5" onSubmit={handleSubmitOtp}>
              ...
              >
                <span className="pointer-events-none absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/25 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
                <span className="relative">{loading ? 'Vérification…' : 'Valider'}</span>
              </button>
