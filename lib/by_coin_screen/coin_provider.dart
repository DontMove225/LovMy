...
import 'dart:convert';
import 'package:lovmy/by_coin_screen/payout_list_api_model.dart';
import 'package:lovmy/by_coin_screen/paystack_api_model.dart';
import 'package:lovmy/by_coin_screen/refer_and_earn_api_model.dart';
...
class ByCoinProvider extends ChangeNotifier {
  ...
  // Stripe Checkout (hosted): returns the checkout_url to load in a webview.
  // itemId is required for type "package"/"plan"; amount is required for type "wallet".
  Future<String?> stripeCheckoutApi({required context, required String type, String? itemId, String? amount}) async{
    Map data = {
      "uid" : Provider.of<HomeProvider>(context,listen: false).uid,
      "type" : type,
    ...
    try{
      var response = await _api.sendRequest.post("${Config.baseUrlApi}${Config.stripeCheckoutApi}",data: data);
      if(response.statusCode == 200 && response.data["Result"] == "true"){
        return response.data["checkout_url"] as String?;
      }else{
        Fluttertoast.showToast(msg: response.data["ResponseMsg"] ?? "Stripe checkout failed");
      }
    }catch(e){
      Fluttertoast.showToast(msg: e.toString());
    }
