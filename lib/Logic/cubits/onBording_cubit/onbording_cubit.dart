...
class OnbordingCubit extends Cubit<OnbordingState> {
  ...
  Future smstypeapi(context) async {
    try {
      ...
      Fluttertoast.showToast(msg: "Connexion au serveur impossible");
      // Ne pas rethrow — laisser le splash continuer via getCurrentLatAndLong
      return null;
    }
  }
  ...
  Future<bool> msgotpapi({required String mobile, required context,}) async {
    ...
    try{
      ...
      if(response.statusCode == 200){
        ...
        if (msgApiModel.result != "true") {
          Fluttertoast.showToast(msg: msgApiModel.responseMsg);
          return false;
        }
        return true;
      }
  ...
  Future<bool> verifyMsgOtpApi({required String mobile, required String otp}) async {
    Map data = {
      'mobile': mobile,
      'otp': otp,
    };
    ...
    try{
      var response = await _api.sendRequest.post("${Config.baseUrlApi}${Config.msgotpverifyapi}",data: data);
      if(response.statusCode == 200){
