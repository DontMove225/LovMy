...
import 'package:lovmy/data/models/mapmodel.dart';
import 'package:lovmy/data/models/usermodel.dart';
...
enum SwipeFeedback { like, pass }

class HomeProvider extends ChangeNotifier {

  late GiftListApiModel giftListApiModel = GiftListApiModel(
    giftlist: [],
    responseCode: '200',
    result: 'false',
    responseMsg: '',
  ...
  Future giftlistApi(context) async{
    try{
      ...
      // Ne pas crasher si l'API cadeaux est indisponible
      print("giftlistApi error: $e");
    }
  }
  ...
  localData(context) {
    Preferences.fetchUserDetails().then((value) {
      try {
        ...
        // Rafraîchir les données depuis le serveur pour avoir la photo à jour
        if (uid != null) {
          _refreshUserFromServer(uid.toString(), context);
          _listenForIncomingMessages(uid.toString());
        }
      } catch (e) {
        print("localData error: $e");
      }
    });
  }

  Future _refreshUserFromServer(String userId, context) async {
    try {
      final response = await _api.sendRequest.post(
        "${Config.baseUrlApi}${Config.userInfo}",
        data: {"uid": userId},
      );
      if (response.statusCode == 200 && response.data["Result"] == "true") {
        // Reconstruire un objet UserLogin à partir de UserData
        final rawData = response.data;
        // Construire la structure attendue par userModelFromJson
        final wrapped = {
          "ResponseCode": "200",
          "Result": "true",
          "UserLogin": rawData["UserData"] ?? rawData["UserLogin"],
        };
        if (wrapped["UserLogin"] != null) {
          userlocalData = userModelFromJson(jsonEncode(wrapped));
          await Preferences.saveUserDetails(wrapped);
          notifyListeners();
        }
      }
    } catch (e) {
      print("_refreshUserFromServer error: $e");
    }
  }

  StreamSubscription? _incomingMessagesSub;
  DateTime? _messageListenStartedAt;

  /// Écoute tous les nouveaux messages Firestore adressés à cet utilisateur
  /// et affiche une notification locale — sans dépendre d'un push serveur
  /// Firebase. Filtré côté client (pas de `where` Firestore) pour éviter
  /// d'avoir besoin d'un index composite.
  void _listenForIncomingMessages(String currentUserId) {
    _incomingMessagesSub?.cancel();
    _messageListenStartedAt = DateTime.now();
    ...
    _incomingMessagesSub = FirebaseFirestore.instance
      ...
      for (final change in snapshot.docChanges) {
        ...
        if (data['reciverId'] != currentUserId) continue;

        final ts = data['timestamp'];
        final sentAt = ts is Timestamp ? ts.toDate() : null;
        if (sentAt == null || _messageListenStartedAt == null || sentAt.isBefore(_messageListenStartedAt!)) {
          continue;
        }

        showLocalNotification(
          title: (data['senderName'] ?? 'Nouveau message').toString(),
          body: (data['message'] ?? '').toString(),
          data: {'id': data['senderid'], 'name': data['senderName']},
        );
      }
    });
  }
  ...
  final Api _api = Api();
  MapModel mapModel = MapModel(profilelist: [], responseCode: '200', result: 'false', responseMsg: '');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (response.data["Result"] == "true") {
          ...
          ensureFakeDatingUserDocs(mapModel.profilelist ?? []);
  ...
  SwipeFeedback? activeSwipeFeedback;

  Future cancleButton(state, context) async {
    // carouselController.jumpToPage(0);
    carouselController.jumpToPage(0);
    interIndex = 0;
    slidecolor = AppColors.steelMidnight;
    activeSwipeFeedback = SwipeFeedback.pass;
  ...
  Future likeButton(state, context) async {
    ...
    activeSwipeFeedback = SwipeFeedback.like;
    notifyListeners();
    animation =
                        ...
                        children: [
                          Expanded(child: MainButton(
                            title: AppLocalizations.of(context)?.translate(
                                "Reset") ?? "Reset",
                            bgColor: AppColors.tenderRose.withOpacity(0.15),
