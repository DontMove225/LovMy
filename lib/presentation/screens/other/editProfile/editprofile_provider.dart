...
class EditProfileProvider extends ChangeNotifier {
 ...
 Future dataTransfer(context)  async {
    ...
    languageList = (userData.userLogin!.language ?? "").isEmpty ? [] : userData.userLogin!.language.toString().split(",");
    interestList = (userData.userLogin!.interest ?? "").isEmpty ? [] : userData.userLogin!.interest.toString().split(",");
    selectReligion = int.tryParse(userData.userLogin!.religion ?? "") ?? -1;
    selectRelationShip = int.tryParse(userData.userLogin!.relationGoal ?? "") ?? -1;
    final otherPic = userData.userLogin!.otherPic;
    if (otherPic == null || otherPic.isEmpty || otherPic == "[]") {
      networkOldImage = [];
    } else if (otherPic.trim().startsWith("[")) {
      try {
        networkOldImage = List.from(jsonDecode(otherPic));
      } catch (_) {
        networkOldImage = [];
      }
    } else {
      networkOldImage = otherPic.split("\$;");
    }
    ...
    ccode = (userData.userLogin!.ccode ?? "91").replaceAll("+", "");
    password.text = "";
    radius = double.tryParse(userData.userLogin!.radiusSearch ?? "") ?? 10;
  }
