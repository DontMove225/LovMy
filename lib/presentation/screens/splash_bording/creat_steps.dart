...
import 'package:lovmy/Logic/cubits/onBording_cubit/onbording_state.dart';
import 'package:lovmy/core/config.dart';
import 'package:lovmy/core/device_locale.dart';
import 'package:lovmy/core/interest_language_icons.dart';
import 'package:lovmy/data/models/languagemodel.dart';
import 'package:lovmy/data/models/relationGoalModel.dart';
import 'package:lovmy/data/models/religionmodel.dart';
...
class _CreatStepsState extends State<CreatSteps> {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Container(
      // Dégradé "Nuit" inversé (obsidienne -> rouge nuit) — même dégradé que
      // la charte graphique, sens inverse pour ce fond d'inscription.
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
          colors: [AppColors.obsidian, AppColors.nightRed],
        ),
      ),
      child: Scaffold(
                   ...
                   if (onbordingCubit.smaTypeApiModel?.smsType == "Msg91") {
                    ...
                    BlocProvider.of<OnbordingCubit>(context).mobileCheckApi(number: onBordingProvider.mobileNumber.text, ccode: onBordingProvider.ccode).then((value) {
                      if (value == "false") {
                        ...
                        onbordingCubit.msgotpapi(mobile: "+${onBordingProvider.ccode + onBordingProvider.mobileNumber.text}", context: context).then((sent) {
                          if (!sent) return;
                                          ...
                                          MainButton(
                                            ...
                                            onTap: () async {
                                              ...
                                              final verified = await onbordingCubit.verifyMsgOtpApi(
                                                mobile: "+${onBordingProvider.ccode}${onBordingProvider.mobileNumber.text}",
                                                otp: onBordingProvider.otp,
                                              );
                        ...
                        Fluttertoast.showToast(msg: "Ce numéro est déjà utilisé");
                      }
                    });
                    ...
                    // Fallback: pas de smsType configuré → utilise Firebase OTP
                    BlocProvider.of<OnbordingCubit>(context).mobileCheckApi(
                      number: onBordingProvider.mobileNumber.text,
                      ccode: onBordingProvider.ccode,
                    ).then((value) {
                      if (value == "false") {
                        BlocProvider.of<OnbordingCubit>(context).sendOtpFunction(
                          number: "+${onBordingProvider.ccode}${onBordingProvider.mobileNumber.text}",
                          context: context,
                          isForgot: false,
                        );
                      } else {
  ...
  Widget step2() {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            AppLocalizations.of(context)?.translate("Your LovMy identity 😎") ?? "Your LovMy identity 😎",
            style: Theme.of(context).textTheme.headlineMedium,
          ...
          IntlPhoneField(
            initialCountryCode: defaultPhoneCountryIso(),
            keyboardType: TextInputType.number,
            ...
            onChanged: (value) {
              onBordingProvider.ccode = value.countryCode.replaceFirst('+', '');
              onBordingProvider.updateNameFiled(
                  controller: onBordingProvider.mobileNumber,
                  value: value.number);
            },
