  ...
  Future<dynamic> isMeassageAvalable(String uid) async {
    ...
    collectionReference.doc(uid).get().then((value) {
      ...
      fields = value.data();
        ...
        isFakeChatPartner = fields["isFake"] == true;
         notifyListeners();
    });
  }
    ...
    if (controller.text.trim().isNotEmpty) {
      collectionReference.doc(resiverUserId).get().then((value) async {
        try{
          ...
          if (fields["isFake"] == true) {
            final realUserId = Provider.of<HomeProvider>(context, listen: false).uid;
            final fakeUserName = fields["name"] ?? "Utilisateur";
            Future.delayed(Duration(seconds: 2 + Random().nextInt(5)), () {
              sendFakeMessage(fakeUserId: resiverUserId, realUserId: realUserId, fakeUserName: fakeUserName);
            });
          }
        }catch(e){
          print("catch condition");
  ...
  void sendPushMessage(String body, String title, String token, BuildContext context) async {
    try {
      final response = await _api.sendRequest.post(
        "${Config.baseUrlApi}${Config.sendPush}",
        data: {
          'token': token,
          'title': title,
          'body': body,
